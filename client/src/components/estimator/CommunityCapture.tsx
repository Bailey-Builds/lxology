...
export default function CommunityCapture() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    // In Beta: log to console; real implementation would POST to an API
    console.log('Community signup:', { email, firstName, role });
    setSubmitted(true);
  }

  if (submitted) {
    return (
      <div className="bg-[#D7E7FF]/40 border border-[#26006B]/20 rounded-2xl p-6 text-center space-y-2">
  ...
  return (
    <div className="bg-[#D7E7FF]/40 border border-[#26006B]/20 rounded-2xl p-6 space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <button
          type="submit"
          className="w-full sm:w-auto px-6 py-2.5 bg-[#FD6A02] hover:bg-[#e05a00] text-white font-semibold rounded-lg text-sm transition-colors"
        >
          Join the Community
        </button>
